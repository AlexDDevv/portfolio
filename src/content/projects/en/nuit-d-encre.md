...
demonstrates: >-
  This project shows the ability to design and evolve an end-to-end typed full-stack application, from database to deployment, over nearly two years. Several choices reflect real command of classic GraphQL and data problems: per-request DataLoaders against N+1 queries, calculations centralized on the server, business constraints enforced in the database, and transactional, idempotent XP granting. The testing strategy is deliberate rather than an afterthought: it targets what is costly when broken (authorization, XP, GDPR, derived display logic) and isolates pure logic so it can be tested without heavy infrastructure. Writing these tests also uncovered and fixed several real bugs along the way. Together with a strict CI, a protected branch and a documented production infrastructure, this approach reflects an engineer who ships code that is maintainable and reliable, not just functional.
gallery:
